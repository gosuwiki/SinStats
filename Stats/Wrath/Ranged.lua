...
function AddonTable.FunctionList.HasteRanged(HUD, data, options, ...)
	...
	hasteRanged = hasteRanged + AddonTable.rangedHaste + AddonTable.hasteEnchants
...
function AddonTable.FunctionList.RDMG(HUD, data, options, ...)
	...
	local avgDamage = (AddonTable.hiDmg + AddonTable.lowDmg + AddonTable.hemoDmg + (AddonTable.hiDmg * AddonTable.bloodFrenzy) + (AddonTable.hiDmg * AddonTable.savageCombat)) / 2
	local returnText
	
	if AddonTable.Band(EB, Max) then returnText = math.floor(AddonTable.hiDmg+1) end
...
function AddonTable.FunctionList.rDPS(HUD, data, options, ...)
	...
	local EB, enhancedStat, baseStat = options.Max_Average_Damage
	local maxDamage = (AddonTable.hiDmg + AddonTable.hemoDmg + (AddonTable.hiDmg * AddonTable.bloodFrenzy) + (AddonTable.hiDmg * AddonTable.savageCombat))
	local lowDamage = (AddonTable.lowDmg + AddonTable.hemoDmg + (AddonTable.lowDmg * AddonTable.bloodFrenzy) + (AddonTable.lowDmg * AddonTable.savageCombat))
	local maxDPS = 0
	local avgDPS = 0
	local BuffColor = ""
	local returnText
