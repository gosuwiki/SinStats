	...
	if NPCguid ~= nil then
		...
		npc_id = tonumber(npc_id)
			if NPCtype == "Creature" then
				if AddonTable.BossArmor[npc_id] then
					AddonTable.targetArmor = AddonTable.BossArmor[npc_id][1]
				end
			end	
	end
end
...
local function itemLevelCheck()
	...
	if mainHandEquipLoc and offHandEquipLoc then
		...
		invSlots = (
			...
			(equippedItemLoc == "INVTYPE_2HWWEAPON")
		) and 17 or 16
	end
...
function AddonTable.StatsCompute()
	...
	AddonTable.innervate, AddonTable.isShapeshift, AddonTable.treeofLife, AddonTable.bladeFlurry, AddonTable.bladeBarrier, AddonTable.renewedHope, AddonTable.windFury = 0, false, 0, 0, 0, 0, 0
	...
	AddonTable.stanceArmorPen, AddonTable.berserker, AddonTable.arcaneEmpowerment, AddonTable.feroInspiration = 0, 0, 0, 0
	...
	elseif (AddonTable.classFilename == "PRIEST") then
		...
		if spellId == 15258 then
			AddonTable.shadowWeaving = count
			AddonTable.shadowWeaving = (AddonTable.shadowWeaving * 2) / 100
		end
