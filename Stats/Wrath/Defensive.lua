...
function AddonTable.FunctionList.DefenseStat(HUD, data, options, ...)
	...
	if options.Display_Basic then
		defenseDisplay = AddonTable.baseDefense
	end
	
	if defenseDisplay >= 540 then
		capColor = AddonTable.greenText
	end
	
	if AddonTable.Band(EB, Total) then
		returnText = capColor .. ("%.1f"):format(defenseDisplay)
	end
	if  AddonTable.Band(EB, Rating) then
		returnText = ("%.0f"):format(AddonTable.defenseRating)
	end
	if  AddonTable.Band(EB, Both) then
		returnText = capColor .. ("%.1f"):format(defenseDisplay) .. " |r(" .. ("%.0f"):format(AddonTable.defenseRating) .. ")"
	end		
...
function AddonTable.FunctionList.Armor(HUD, data, options, ...)
	...
	if AddonTable.armorDebuff < 0 then
		debuffColor = AddonTable.redText
	end	
	
	if AddonTable.Band(EB, Enhanced) then
		returnText = debuffColor .. ("%.0f"):format(AddonTable.armorValue)
	end
	if  AddonTable.Band(EB, Base) then
		returnText = ("%.0f"):format(AddonTable.armorValue)
	end
	if  AddonTable.Band(EB, Both) then
		returnText = debuffColor .. ("%.0f"):format(AddonTable.armorValue) .. "/|r" .. AddonTable.armorValue
	end	
...
function AddonTable.FunctionList.Resilience(HUD, data, options, ...)
	...
	if AddonTable.Band(EB, Critical) then
		percentStat = Resilience
	end
	if  AddonTable.Band(EB, CritDamage) then
		ratingStat = resiDamage
	end
...
function AddonTable.FunctionList.Mitigation(HUD, data, options, ...)
	...
	if AddonTable.targetLevel < 80 then
		armorReduction = (AddonTable.armorValue/(AddonTable.armorValue+400+85*((5.5 * AddonTable.targetLevel)-265.5))) * 100
	elseif AddonTable.targetLevel == 80 then
		armorReduction = AddonTable.armorValue/(AddonTable.armorValue + 15232.5) * 100
	end	
	...
	elseif GetShapeshiftForm() == 2 and (AddonTable.classFilename == "DEATHKNIGHT") then
		armorReduction = armorReduction + 8
	end
	...
	if armorReduction >= 75 then
		debuffColor = AddonTable.greenText
	elseif armorReduction < 0 then
		armorReduction = 0
		debuffColor = AddonTable.redText
	end
...
function AddonTable.FunctionList.CritReceived(HUD, data, options, ...)
	...
	chanceGetCrit = (uncritCap + getCritLevel) - resilCrit - AddonTable.survFittest - AddonTable.Resilvalue - AddonTable.SleightTalent - AddonTable.moltenArmor
	...
	if AddonTable.Band(EB, Chance) then
		returnText = debuffColor .. ("%.2f%%"):format(chanceGetCrit)
	end
	if  AddonTable.Band(EB, Reduction) then
		returnText = ("%.2f%%"):format(critTakenCap)
	end
	if  AddonTable.Band(EB, Both) then
		returnText = debuffColor .. ("%.2f%%"):format(chanceGetCrit) .. " |r(" .. ("%.2f%%"):format(critTakenCap) .. ")"
	end		
