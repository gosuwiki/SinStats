...
function AddonTable.SpellUpdate()
	...
	AddonTable.masterSucc + AddonTable.totemWrath + AddonTable.innerFocus + AddonTable.heartCrusader + AddonTable.wintersChill + AddonTable.impSorch + AddonTable.impShadowbolt + AddonTable.faerieFireImp
...
function AddonTable.FunctionList.Holy(HUD, data, options, ...)
	...
	(holySpell * AddonTable.arcaneEmpowerment) + (holySpell * AddonTable.feroInspiration))
...
function AddonTable.FunctionList.Nature(HUD, data, options, ...)
	...
	(natureSpell * AddonTable.ebonPlague) + (natureSpell * AddonTable.earthMoon) + (natureSpell * AddonTable.arcaneEmpowerment) + (natureSpell * AddonTable.feroInspiration))
...
function AddonTable.FunctionList.Shadow(HUD, data, options, ...)
	...
	(shadowSpell * AddonTable.arcaneEmpowerment) + (shadowSpell * AddonTable.feroInspiration))
...
function AddonTable.FunctionList.Frost(HUD, data, options, ...)
	...
	(frostSpell * AddonTable.arcaneEmpowerment) + (frostSpell * AddonTable.arcanePower) + (frostSpell * AddonTable.feroInspiration))
...
function AddonTable.FunctionList.Arcane(HUD, data, options, ...)
	...
	(arcaneSpell * AddonTable.arcaneEmpowerment) + (arcaneSpell * AddonTable.arcanePower) + (arcaneSpell * AddonTable.feroInspiration))
...
function AddonTable.FunctionList.HasteCaster(HUD, data, options, ...)
	...
	if AddonTable.classFilename == "WARLOCK" then
		local _, _, _, mainHandEnchantID  = GetWeaponEnchantInfo()
		if AddonTable.SpellStones[mainHandEnchantID] then
			spellStone = hasteBonus - hasteSpell
			spellStone = math.abs(spellStone)
		end
	end
	
	hasteSpell = hasteSpell + AddonTable.castHaste + AddonTable.earthMother + AddonTable.eleMasteryHaste + AddonTable.Enlightenment + spellStone + AddonTable.NetherwindPresence +
				 AddonTable.borrowedTime + AddonTable.maelstromWeapon
	
	if options.Display_Basic then hasteSpell = GetHaste() end		

	if AddonTable.Band(EB, Percentage) then enhancedStat = hasteSpell end
